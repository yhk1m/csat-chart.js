...
그래서 «폭이 40% 넓은 글꼴» 을 흉내 내는 판을 검사에 하나 더 두었다
(`measureText().width` 만 부풀리면 배치를 정하는 쪽과 재는 쪽이 함께 그 글꼴을
본다). 1.05배부터 1.5배까지 훑어 처음 깨지는 지점을 찾고, 1.5배에서도 아무 데도
안 넘치는 것을 확인한 뒤 검사에 거는 값으로 1.4 를 골랐다 — CI 글꼴이 이미 5%쯤
넓으니 거기서는 사실상 1.47배가 된다. 그 판으로 같은 종류가 일곱 자리 더 나왔고,
함께 고쳤다.
