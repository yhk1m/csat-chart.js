...
> **기록물 안내(1.1.0):** 이 문서는 1.0.0 을 내기 전에 쓴 설계다. 아래 §6 의
> **「`core/`는 손대지 않는 층이다」는 이식이 끝날 때까지의 규칙이었고 1.1.0 에서
> 끝났다.** 같은 대목이 예고한 대로(«`core/`를 고쳐야 하는 상황이 생기면 그 자체를
> 별도 판단 지점으로 삼고 CHANGELOG에 남긴다») 막대 폭을 고치면서 처음 손댔다.
> 골든 이미지의 역할도 그때 바뀌었다 — 이식의 증거에서 렌더 결과의 기준으로.
