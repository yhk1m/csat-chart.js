...
**경제 좌표평면의 물결 자리를 자료가 정한다.** 물결(축 생략 기호 `≈`)은 1.4.0
부터 그렸지만 자리가 코드에 박혀 있었다 — 원점과 첫 눈금의 한가운데(2026학년도
9월 7번 실측), 눈금이 하나도 없으면 축 길이의 12%. 실물 한 장에서 잰 자리라 다른
그림에는 안 맞는다.

| 칸 | 값 | 하는 일 |
|---|---|---|
| `xAxis.brokenAt`·`yAxis.brokenAt` | `71` | 물결을 놓을 **축의 값**. 미지정이면 예전 자리 그대로 |

**자리를 축의 «값» 으로 말한다.** 이 라이브러리는 눈금에서 이미 「값이 곧
자리다」를 못박아 두었으므로(`ticks`), 비율이나 픽셀 같은 둘째 자[尺]를 들이지
않았다. `min`~`max` 밖을 적으면 축 안으로 자른다 — 물결은 축 «위» 의 기호라 축
밖에 놓을 자리가 없다. 숫자가 아닌 값(NaN·Infinity)은 «적지 않음» 과 같게 본다.
`broken` 이 거짓이면 읽지 않는다.

새 기준 이미지는 `econPlaneBrokenAt.png` 한 장이고, **먼저 있던 41장은 바이트
그대로다** — 적지 않으면 예전 자리라는 것의 증거다.

데모 카드에도 「가로축 물결 자리」·「세로축 물결 자리」 칸이 났다. 비우면 기본
자리로 돌아가고, 물결을 끄면 칸 자체가 사라진다.

## 1.6.1 — 2026-09-11

**격자 굵기 칸을 하나 더 냈다.** 1.6.0 의 `gridColor` 로 격자를 진하게 해도 0.5px 라
시험지의 점선보다 가늘었다. 두 장 모두 격자가 1px 쯤이다.

| 칸 | 값 | 하는 일 |
|---|---|---|
| `gridWidth` | `1` | 가로·세로 격자 굵기(px). 미지정이면 0.5 — 예전 그대로 |

`lineLeader.png`·`lineEndExam.png` 두 장은 `gridWidth: 1` 로 다시 만들었고, 나머지
39장은 바이트 그대로다. `drawYAxis` 도 `gridWidth` 인자를 받는다(기본 0.5).

## 1.6.0 — 2026-09-11

**꺾은선을 실물 두 장만큼 더 그릴 수 있게 했다.** 2027학년도 6월 모평 한국지리
17번(세 지역의 인구 변화)과 2026학년도 9월 모평 한국지리 13번(권역별 인구 변화)을
재어 보니, 기존 `line` 에 네 가지가 빠져 있었다.

### 새로 그릴 수 있는 것

| 칸 | 값 | 어느 그림에서 왔나 |
|---|---|---|
| `xLabels[]` 의 빈 문자열 | `''` | 17번 — 자료는 5년마다인데 라벨과 세로 격자는 10년마다(1980·1990·…). 빈 이름 자리는 이름도 격자도 건너뛴다. 라벨을 솎아 내는 자동 걸러내기도 **이름이 있는 자리들 사이의 간격**으로 판단한다 — 칸 간격으로 재면 이름 있는 자리까지 엉뚱하게 빠진다 |
| `series[].stroke` · `series[].lineWidth` | `'#999'` · `3.5` | 17번 — (가)는 굵은 일점쇄선, (나)는 굵은 **회색** 실선, (다)는 가는 검정 실선. 흑백 시험지가 계열을 가르는 세 번째 수단이 굵기와 회색이다 |
| `labelPlacement: 'leader'` · `series[].leader` | `{ at, dx, dy }` | 17번 — 이름을 선 끝이 아니라 선 **근처**에 쓰고 짧은 유도선으로 가리킨다. `at` 은 가리킬 점, `dx`·`dy` 는 그 점에서 라벨까지의 거리. 안 주면 가운데 점의 오른쪽 위. `at` 이 범위를 벗어나면 끝으로 당기고, 그 점에 값이 없으면 값이 있는 가장 가까운 점을 가리킨다 |
| `xGrid` · `gridColor` | `true` · `'#555'` | 두 장 모두 — 가로·세로 진한 점선 격자. 예전에는 가로 격자만, 그것도 연회색이었다 |

`sourceInline` 도 `line` 이 읽는다 — 두 장 모두 출처가 마지막 각주와 같은 줄 오른쪽
끝에 있다. 「알려진 한계」의 목록에서 `line` 을 뺐다.

### 기준 이미지

`line.png` 는 **바이트 그대로**다 — 새 칸을 안 적으면 예전과 같이 그린다는 증거다.
`lineLeader.png`(17번 모양)·`lineEndExam.png`(13번 모양) 두 장이 늘어 41장이다.

### 그 밖에

- `labelPlacement: 'legend'` 일 때 범례 상자의 선 아이콘이 **선과 같은 굵기**로
  그려진다(`lineWidth`, 기본 2). 예전에는 범례 도우미의 기본값 2.5px 이었다 —
  반 픽셀 차이지만 골든이 안 지키는 자리라 여기 적어 둔다.
- `drawYAxis` 가 `gridColor` 인자를 받는다(기본 `#ccc` 그대로). 저수준 렌더러를
  직접 쓰는 사람에게만 보이는 변화다.
- 같은 확장을 GeoGrapher(geographer.bgnl.kr)에도 넣었다 — 거기서는 폼으로 값을
  넣고 PNG 로 뽑는다.

## 1.5.1 — 2026-09-09

**npm 설명문만 영어로 바꿨다.** 그리는 코드는 한 줄도 안 건드렸다.

`package.json` 의 `description` 은 npm 검색 결과와 패키지 쪽에 뜨는 한 줄이다.
영어권에서도 무엇인지 읽히도록 «JavaScript library for creating South Korean
exam-style and educational charts» 로 바꿨다. 이 한 줄은 패키지 안에 담겨
올라가므로, 1.5.0 을 이미 발행한 뒤로는 판을 하나 올려야 반영된다 — 그것이
이 판이다.

README 첫 줄과 코드 주석은 한국어 그대로다. 실제로 쓰는 사람이 한국 선생님들
이기 때문이다.
